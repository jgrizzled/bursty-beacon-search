...
The transformation family should also commute with the propagation channel. Delays, amplitude scalings, and polarization rotations commute with an unknown linear time-invariant channel: a common scattering convolution applies identically to all components, so pairwise similarity survives without deconvolution. Time reversal does not—the reversed copy convolved with a causal scattering kernel is not the reversal of the convolved original—so reversal tags degrade exactly where scattering is strong and are recoverable only after channel deconvolution. A rational builder restricts tags to channel-commutative transformations, and the archival search should restrict its primary transformation space the same way, which directly reduces the trials factor of Section 10.7 rather than merely calibrating around it.
