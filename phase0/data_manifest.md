...
- S1: pass (with the frozen construction rule). S2: 69 events after the frozen composition (60 Nimmo + B1–B9; see 1.7a) — pass; **statistical-power caveat: 53/69 bursts lie within one ~2 h storm session on 2022-01-14**, so most events constrain only within-session structure; recorded for the null-interpretation section, not a sample-eligibility issue. S3: 46 composed sessions, ~495 d — pass. S4: no published activity-window period (periodicity searches on published TOAs are null) — pass. S5: mas-scale VLBI localization — pass.
...
- Status: **extraction complete (2026-08-07)** — tarballs fetched and hashed (Section 1.6), extraction implemented in `scripts/extract_session_logs.py` (`build_eff20200120E_*`), frozen handling rules resolved as recorded in 1.7a.

### 1.7a FRB 20200120E extraction outcomes (2026-08-07; the frozen-handling decisions of 1.7 as implemented)

- **(a) Session-window construction (Pearlman precedence):** combined Effelsberg session list = **46 sessions**, MJD 59199.089–59693.690 (~495 d): 15 Pearlman on-source UTC windows (Dec 2020–May 2021) + 3 Kirsten-only PRECISE runs (PR161A, PR163A, PR166A) + 28 Nimmo-only sessions. Pearlman's log adds **4 sessions absent from Nimmo's 42-row table** — two Dec 2020 sessions (extending the baseline from ~428 to ~495 d) and, critically, the 2021-04-30→05-01 EDD session containing bursts B6–B9 plus a 2021-05-14 EDD session. Where logs overlap, per-session burst counts agree exactly (asserted in code). Nimmo's flat "4.99 hr" PRECISE durations are nominal scheduled blocks (e.g. PR158A actually ran 11:00–22:00 UT with Effelsberg on-source 11:08–15:42 per Pearlman) — the precedence rule exists for exactly this.
- **(a′) PR163A resolution (frozen):** Kirsten logs PR163A as MJD 59347.417–59347.625; Nimmo lists a session *starting* 59347.625 — Nimmo's start equals Kirsten's stop, whereas in all ten other shared runs Nimmo's start equals Kirsten's start. **Adopted: the Kirsten window** (original PRECISE run report; the Nimmo table is a re-compilation citing it). Zero bursts under either reading; the choice moves 5 h of burst-free exposure by 5 h; the rejected alternative window is recorded in the output row's notes.
- **(b) Burst deduplication:** frozen event list = **69 unique bursts** (not 60 as pre-extraction estimated): Pearlman B1–B9 + Nimmo 2023's 60 storm-era bursts. B1–B5 deduplicated by published ID against Kirsten 2022 (and Nimmo 2022); Pearlman's re-barycentred TOAs adopted, superseded Kirsten TOAs retained in a dedicated column — measured shifts 1.2–4.2 ms, consistent with the ~3–4 ms superseded-position estimate. B6–B9 appear only in Pearlman and fall inside the Pearlman-only 2021-04-30 window; excluding them while including their session would bias W(t), so they enter the event list (S2 remains a pass; the storm caveat becomes **53/69** bursts in one ~2 h session). Inbound TOA conventions recorded: Nimmo TDB/infinite-frequency/VLBI position (ephemeris unstated); Pearlman TDB/infinite-frequency/corrected VLBI position/**DE405** — heterogeneity handled by the prereg's frozen conversion procedure.
- **(c) Non-Effelsberg PRECISE runs:** PR160A/PR162A/PR164A/PR165A **dropped** (conservative rule c) — verified that the Kirsten publication assigns no station or on-source semantics to these runs (stations are pinned only for the three detection epochs via EVN codes); they overlap no Effelsberg log row.
- All rules are implemented as asserted code, not prose; `PROVENANCE.json` records input/output SHA-256, row counts, and construction notes.
...
| arXiv source | `2206.03759.tar.gz` (Nimmo 2023 obs + burst tables) | 3,245,246 | `af235bc3…21c8c545` | arXiv e-print (fetched 2026-08-07) |
| arXiv source | `2105.11445.tar.gz` (Kirsten 2022 PRECISE log + B1–B5) | 5,879,235 | `a9c37bbf…c50b4b21` | arXiv e-print (fetched 2026-08-07) |
| arXiv source | `2308.10930.tar.gz` (Pearlman 2024 radio-obs log + B1–B9) | 1,268,340 | `79894fbf…066faefe` | arXiv e-print (fetched 2026-08-07) |
...
**FRB 20200120E composed window and burst lists (extracted 2026-08-07):** `phase1/windows/effelsberg20200120E_sessions.csv` (46 sessions) and `effelsberg20200120E_bursts.csv` (69 bursts), produced by the `build_eff20200120E_*` functions of the frozen extraction script under the Section 1.7 handling rules; construction details and outcomes recorded in Section 1.7a below and in `PROVENANCE.json`.
...
| 2026-08-07 | FRB 20200120E freeze extraction: 2206.03759/2105.11445/2308.10930 tarballs downloaded, hashed, and their four log/burst tables extracted and composed (Section 1.7a); table rows viewed to write and validate parsers; cross-table window overlaps and per-session burst-count reconciliation computed; B1–B5 TOA shifts (1.2–4.2 ms) computed for dedup documentation | Checksum recording and extraction-script validation required by the freeze checklist and manifest 1.7 | Session logs are not burst data; burst-table handling limited to ID dedup, session assignment, and pair-wise superseded-TOA offsets; no periodicity, clustering, or timing-structure statistics computed |
