...
- [ ] Run and commit the synthetic acceptance tests (Section 9) at full scale. _Status 2026-08-07 (evening): the exact C kernel (`phase1/_scankernel.c`, equivalence-validated by `scripts/validate_fastscan.py` — all pass, incl. full-scale M2 cross-check) brings a full-scale Λ to 1,233 s (~235× over the reference); the frozen suite (1,000 sims/family + 200+200 injections) ≈ 1,570 core-hours. **The full-scale run is launched** (checkpointed JSONL, resumable via deterministic per-task seeding; 7 workers, detached with caffeinate; log `phase1/acceptance_full_run.log`), ETA ≈ 9 days locally — or the checkpointable harness can be moved to a ~$100 cloud box for ~1-day turnaround. Smoke scale passes under the same kernel path (10/10 checks, artifact committed). Two pre-freeze harness corrections recorded in the 2026-08-07 kernel commit: prereg-4.1 event-in-window invariant asserted (degenerate zero-exposure cells are fp-accidents of the reference, unreachable on physical streams), and t5 jitter clipped to session bounds. The post-tag Section 6.2 calibration (~50–100× this cost) becomes cloud-feasible (~$1–3k spot) with the same kernel; its execution plan is decided at tag time._
