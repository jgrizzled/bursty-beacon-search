...
- **FRB 20200120E — Effelsberg monitoring campaign** (Nimmo et al. 2023, MNRAS 520, 2281; Kirsten et al. 2022 and Pearlman et al. 2024 as log/TOA supplements): 42-session Effelsberg log with non-detection sessions over a ~428-day baseline + 60 barycentric TDB infinite-frequency TOAs, both complete LaTeX tables in the arXiv sources. **Verified 2026-08-07** in the rev3-motivated nearest-host pass (added pre-freeze, before any period-search output existed for this source; manifest 1.7). This is the thesis-preferred nearest-host target (M81, 3.6 Mpc). S4 pass: no published activity-window period. Frozen-handling requirements (session-window construction precedence, cross-table burst deduplication, superseded-position TOA refinements) and the statistical-power caveat that 53/60 bursts lie in a single ~2 h storm session are recorded in manifest 1.7.
...
- [ ] Pin software environment (astropy/erfa/numpy/scipy versions, ephemeris file hash, repo commit).
- [ ] Compute and commit per-source alias sets and grids (Section 5) from the verified windows.
- [ ] Run and commit the synthetic acceptance tests (Section 9).
- [ ] Record the literature-search protocol appendix in [novelty_audit.md](novelty_audit.md).
