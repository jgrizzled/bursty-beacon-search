...
- [ ] Run and commit the synthetic acceptance tests (Section 9) at full scale. _Status 2026-08-07: smoke scale passes; the exact fast reimplementation (`phase1/fastscan.py`, equivalence-validated) brings a full-scale Λ evaluation to 138 min single-core (~35× over the reference), but the frozen full-scale suite (1,000 sims/family + injections ≈ 4,600 Λ evaluations) totals ≈ 10,600 core-hours — beyond a single workstation. Execution strategy (distributed compute vs. further kernel engineering vs. pre-freeze scale revision) is an open decision recorded before the tag; the same decision governs the ~100× larger post-tag Section 6.2 calibration._
